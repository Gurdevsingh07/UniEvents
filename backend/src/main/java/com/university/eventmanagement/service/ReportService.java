...
public class ReportService {
    ...
    public byte[] generatePdfReport(Long eventId) {
        ...
        try {
            ...
            document.add(
                    new Paragraph("Date: " + event.getEventDate().format(DateTimeFormatter.ofPattern("dd MMM yyyy"))
            ...
            for (Registration reg : registrations) {
                ...
                table.addCell(
                        new Cell().add(new Paragraph(String.valueOf(index++)).setFont(normalFont).setFontSize(9)));
                table.addCell(new Cell().add(new Paragraph(student.getFullName()).setFont(normalFont).setFontSize(9)));
                table.addCell(
                        new Cell().add(new Paragraph(student.getStudentId() != null ? student.getStudentId() : "N/A")
                                .setFont(normalFont).setFontSize(9)));
                table.addCell(
                        new Cell().add(new Paragraph(student.getDepartment() != null ? student.getDepartment() : "N/A")
                                .setFont(normalFont).setFontSize(9)));
                table.addCell(new Cell().add(new Paragraph(present ? "Present (" + checkInTime + ")" : "Absent")
                        .setFont(normalFont).setFontSize(9)));
            }
    ...
    public byte[] generateExcelReport(Long eventId) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            String[] columns = { "S.No", "Name", "Student ID", "Email", "Department", "Status", "Check-in Time" };
            ...
            for (Registration reg : registrations) {
                ...
                row.createCell(2).setCellValue(student.getStudentId() != null ? student.getStudentId() : "N/A");
                row.createCell(3).setCellValue(student.getEmail());
                row.createCell(4).setCellValue(student.getDepartment() != null ? student.getDepartment() : "N/A");
